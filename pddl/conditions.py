...
class ConstantCondition(Condition):
    ...
    def literal_replaced(self, replace_dict):
        return self
...
class Truth(ConstantCondition):
    ...
    def instantiate(self, var_mapping, init_facts, fluent_facts, result):
        pass
    def instantiate_full(self, var_mapping, result):
        pass
    def negate(self):
        return Falsity()
...
class JunctorCondition(Condition):
    ...
    def literal_replaced(self, replace_dict):
        return self.__class__([part.literal_replaced(replace_dict) for part in self.parts])
...
class Conjunction(JunctorCondition):
    ...
    def instantiate_full(self, var_mapping, result):
        for part in self.parts:
            part.instantiate_full(var_mapping, result)

    def negate(self):
        return Disjunction([p.negate() for p in self.parts])

    def output_formula(self):
        return "&".join([ part.output_formula() for part in self.parts])

    def to_smt(self):
        return "(and %s)"%" ".join([part.to_smt() for part in self.parts])

    def to_ispl(self, varas_dict):
        return " and ".join([part.to_ispl(varas_dict) for part in self.parts])
...
class Disjunction(JunctorCondition):
    ...
    def output_formula(self):
        return "|".join([ "(%s)"%part.output_formula() for part in self.parts])

    def to_smt(self):
        return "(or %s)"%" ".join([part.to_smt() for part in self.parts])

    def to_ispl(self, varas_dict):
        return "(%s)"%" or ".join([part.to_ispl(varas_dict) for part in self.parts])
...
class QuantifiedCondition(Condition):
    ...
    def literal_replaced(self, replace_dict):
        return self.__class__(self.parameters, [part.literal_replaced(replace_dict) for part in self.parts]) 
...
class UniversalCondition(QuantifiedCondition):
    ...
    def output_formula(self):
        ...
        arglist = ", ".join(map(str, self.parameters))
        body = "&".join([ part.output_formula() for part in self.parts])
        return "forall(%s)[%s]"%(arglist,body )

    def to_smt(self):

        var_str = " ".join(["(%s %s)"%(para.name, para.type_name) for para in self.parameters])
        part_str = " ".join([part.to_smt() for part in self.parts])
        return "(forall (%s) %s)"%(var_str, part_str)
...
class ExistentialCondition(QuantifiedCondition):
    ...
    def output_formula(self):
        ...
        arglist = ", ".join(map(str, self.parameters))
        body = "&".join([ part.output_formula() for part in self.parts])
        return "exists(%s)[%s]"%(arglist,body )

    def to_smt(self):

        var_str = " ".join(["(%s %s)"%(para.name, para.type_name) for para in self.parameters])
        part_str = " ".join([part.to_smt() for part in self.parts])
        return "(exists (%s) %s)"%(var_str, part_str)
...
class Atom(Literal):
    ...
    def output_formula(self):
        return "%s(%s)"%(self.predicate, ','.join(list(self.args)))

    # simplified
    def change_parts(self, parts):
        
        if self.predicate == "=" and self.args[0] == self.args[1]:
            return Truth()
        else:
            return self
    ...
    def to_ispl(self, varas_dict):
        return "%s=true"%varas_dict[self]

    def literal_replaced(self, replace_dict):
        #print("------axiom_replaced:", str(self), str(self) in axioms_dict)
        #print(axioms_dict.keys())
        # print("_------")
        # print(str(self))
        # print("_------")
        # print(axioms_dict)
        
        if str(self) in replace_dict:
            return replace_dict[str(self)]
        elif self.predicate.find("new-axiom@") == 0:
            # print("hellosssss")
            # exit(0)
            return Falsity()
        else:
...
class NegatedAtom(Literal):
    ...
    def to_smt(self):
        if len(self.args) == 0:
            ...
            return "(not (%s %s))"%(self.predicate, " ".join(self.args))

    def to_ispl(self, varas_dict):
        return "%s=false"%varas_dict[self.negate()]
