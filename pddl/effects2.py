...
class Effect:
    ...
    # new adding
    def get_positve_condition(self, fluent):
        if fluent.name == self.literal.name and isinstance(self.literal, conditions.Atom):
            return self.condition
        else:
            return conditions.Falstify()

    # new adding
    def get_negative_condition(self, fluent):
        if fluent.name == self.literal.name and isinstance(self.literal, conditions.NegatedAtom):
            return self.condition
        else:
            return conditions.Falstify()
...
class ConditionalEffect:
    ...
    # new adding
    def get_positve_condition(self, fluent):
        has_condition = self.effect.get_positve_condition(fluent)
        if has_condition.simplified()!=conditions.Falstify():
            return self.condition
        else:
            return conditions.Falstify()

    def get_negative_condition(self, fluent):
        has_condition = self.effect.get_negative_condition(fluent)
        if has_condition.simplified()!=conditions.Falstify():
            return self.condition
        else:
            return conditions.Falstify()
...
class UniversalEffect:
    ...
    # new adding
    def get_positve_condition(self, fluent):
        return self.effect.get_positve_condition(fluent)

    def get_negative_condition(self, fluent):
        return self.effect.get_negative_condition(fluent)
...
class ConjunctiveEffect:
    ...
    # new adding
    def get_positve_condition(self, fluent):
        positive_conditions = [ effect.get_positve_condition(fluent) for effect in self.effects]
        return conditions.Disjunction(positive_conditions)


    def get_negative_condition(self, fluent):
        negative_conditions = [ effect.get_negative_condition(fluent) for effect in self.effects]
        return conditions.Disjunction(negative_conditions)
...
class SimpleEffect:
    ...
    # new adding
    def get_positve_condition(self, fluent):
        if fluent.name == self.effect.name and isinstance(self.effect, conditions.Atom):
    ...
    # new adding
    def get_negative_condition(self, fluent):
        if fluent.name == self.effect.name and isinstance(self.effect, conditions.NegatedAtom):
            return conditions.Truth()
        else:
            return conditions.Falstify()
