...
class Action:
    ...
    def uniquify_variables(self, type_map={}):
        ...
        self.type_map.update({par.name: par.type_name for par in self.parameters})
    ...
    def instantiate_with_paras(self, parameters):
        var_mapping = {par.name: arg for par, arg in zip(self.parameters, parameters)}
        return self.instantiate_full(var_mapping)

    def instantiate_full(self, var_mapping):
        arg_list = [var_mapping[par.name]
                    for par in self.parameters[:self.num_external_parameters]]
        name = "(%s %s)" % (self.name, " ".join(arg_list))

        precondition = []
        self.precondition.instantiate_full(var_mapping, precondition)
        effects = []
        for eff in self.effects:
            eff.instantiate_full(var_mapping, effects)
        return PropositionalAction(name, precondition, effects, 0)
...
class PropositionalAction:
    ...
    def to_ispl_action(self):
        return self.name.strip("(").strip(")").strip(" ").replace("-", "__")


class PropositionalSimpleAction:
    def __init__(self, name, precondition, effects, cost):
        self.name = name
        self.precondition = precondition
        self.add_effects = []
        ...
        for literal in effects:
            if not literal.negated:
                self.add_effects.append(literal)
        # Warning: This is O(N^2), could be turned into O(N).
        # But that might actually harm performance, since there are
        # usually few effects.
        # TODO: Measure this in critical domains, then use sets if acceptable.
        for literal in effects:
            if literal.negated and literal.negate() not in self.add_effects:
                self.del_effects.append(literal.negate())
        self.cost = cost

    def __repr__(self):
        return "<PropositionalSimpleAction %r at %#x>" % (self.name, id(self))
